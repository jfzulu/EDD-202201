...
void printCodes(struct MinHeapNode *root, int arr[], int top,
                std::vector<Dato> &codigos) {

  // Assign 0 to left edge and recur
  if (root->left) {

    arr[top] = 0;
    printCodes(root->left, arr, top + 1, codigos);
  }
  ...
  if (root->right) {
    ...
    arr[top] = 1;
    printCodes(root->right, arr, top + 1, codigos);
  }
  ...
  if (isLeaf(root)) {
    ...
    // printArr(arr, top);
    Dato data;
    data.intensidad = root->data;
    for (int i = 0; i < top; ++i) {
      data.codigo[i] = arr[i];
    }
    data.tam= top;
    codigos.push_back(data);
  }
}
...
std::vector<Dato> HuffmanCodes(int data[], int freq[], int size) {
  ...
  std::vector<Dato> codigos;
  printCodes(root, arr, top, codigos);
